...
export default function EventTransportPage() {
  ...
  const saveVehicle = (payload, vehicleId) => vehicleId ? updateVehicleMutation.mutateAsync({ vehicleId, ...payload }) : createVehicleMutation.mutateAsync(payload);
  ...
  return <div className="min-w-0 space-y-4"><EventFunctionsHeader booking={booking} metrics={metrics} showMetrics={false} onBack={() => navigate('/dashboard/assigned-events')} onEdit={() => setEditBookingOpen(true)} onMarkReady={() => readyMutation.mutate()} onOpenPlanning={() => document.getElementById('transport-plan')?.scrollIntoView({ behavior: 'smooth' })} /><EventDetailTabs activePrimary="plan" activePlan="guests" showPlanTabs onSelect={selectEventTab} /><EventTransportPanel assignments={assignments} guests={guests} vehicles={vehicles} onGuestTab={selectGuestTab} onAddTransport={() => setTransportDialog({})} onEditTransport={setTransportDialog} onAddVehicle={() => setVehicleDialog({})} onEditVehicle={setVehicleDialog} /><EventVehicleDialog open={Boolean(vehicleDialog)} onOpenChange={(open) => !open && setVehicleDialog(null)} item={vehicleDialog} vehicles={vehicles} saving={createVehicleMutation.isPending || updateVehicleMutation.isPending} onSave={saveVehicle} /><EventTransportDialog open={Boolean(transportDialog)} onOpenChange={(open) => !open && setTransportDialog(null)} item={transportDialog} guests={guests} vehicles={vehicles} functions={functions} allocations={booking.stayAllocations || []} properties={booking.accommodationProperties || []} saving={createTransportMutation.isPending || updateTransportMutation.isPending} onSave={saveTransport} /><EditBookingDialog open={editBookingOpen} onOpenChange={setEditBookingOpen} booking={booking} employees={employees} saving={updateBookingMutation.isPending} onSave={(payload) => updateBookingMutation.mutate(payload)} /></div>;
}
