...
export default function EventHospitalityPanel({
  ...
  const filtered = useMemo(() => requirements.filter((item) => {
    ...
    if (ownerFilter !== 'all' && (idOf(item.owner) || 'unassigned') !== ownerFilter) return false;
    if (term && !searchable.includes(term)) return false;
    if (functionFilter !== 'all' && functionIds.length && !functionIds.includes(functionFilter)) return false;
    if (statusFilter !== 'all' && item.status !== statusFilter) return false;
    return true;
  }), [functionFilter, functionMap, ownerFilter, requirements, search, statusFilter]);

  const pageCount = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const pageRows = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  useEffect(() => setPage(1), [functionFilter, ownerFilter, search, statusFilter]);
  useEffect(() => {
  ...
  const filterActions = (
    <>
      ...
      <Select value={functionFilter} onValueChange={setFunctionFilter}>
        <SelectTrigger className="h-9 w-full text-xs lg:w-32"><SelectValue /></SelectTrigger>
        <SelectContent>
  ...
  return (
    <Card id="hospitality-plan" className="crm-card overflow-hidden">
      <EventGuestsNav active="hospitality" onSelect={onGuestTab} />
      <CardContent className="p-0">
        <div className="grid grid-cols-2 items-center gap-2 border-b border-border px-4 py-3 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto_auto]">
          {filterActions}
          <Button type="button" size="sm" className="h-9 gap-2" variant="custom" onClick={onAdd}>
          ...
          <Table className="min-w-[1050px] text-xs">
            <TableHeader>
              <TableRow className="bg-muted/30 hover:bg-muted/30">
                <TableHead className="pl-6 text-center">Requirement</TableHead>
                    ...
                    <TableCell className="pl-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {item.requirementType ? <p className="mt-0.5 text-[10px] text-muted-foreground">{item.requirementType}</p> : null}
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="max-w-48"><div className="flex flex-wrap gap-1">{coverageLabel(item, functionMap).split(', ').map((name) => <Badge key={name} variant="secondary" className="rounded px-1.5 py-0.5 text-[10px] font-normal">{name}</Badge>)}</div></TableCell>
