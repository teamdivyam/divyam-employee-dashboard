...
export default function AddExpenseDialog({
  ...
  profile,
  additionalFields,
  paymentSourceOptions = PAYMENT_SOURCE_OPTIONS.slice(1),
  categoryOptions = CATEGORY_OPTIONS.slice(1),
  attachmentHint = "PNG, JPG, JPEG, HEIF or HEIC · Up to 10 files · 5 MB each",
  attachmentTypes = ALLOWED_ATTACHMENT_TYPES,
  allowSubmit = false,
  showDraft = true,
}) {
  const fileInputRef = useRef(null);
  const employeeName = profile?.name || employee?.fullName || employee?.name || "Employee";
  const employeeRole = profile?.detail || employee?.designation || employee?.jobTitle || employee?.role || "Employee";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] max-w-2xl gap-0 overflow-hidden border-border bg-card p-0 text-card-foreground">
        <form
          className="flex min-h-0 max-h-[92dvh] flex-col overflow-hidden"
          onSubmit={(event) => {
        ...
        >
          ...
          <fieldset disabled={submitting} className="min-w-0 space-y-2.5 pb-3">
            <div className="sticky top-0 z-10 flex flex-wrap items-center gap-2 rounded-md border border-border bg-card p-2 shadow-sm">
            ...
            {form.paymentSource === "Office Expense Advance" && availableAdvance != null ? (
              <div>
            ...
            <div className="space-y-2 rounded-b-md border border-t-0 border-border p-3">
              ...
              >
                ...
                <span>
                  ...
                  <span className="block text-[10px] text-muted-foreground">{attachmentHint}</span>
                </span>
              </button>
          ...
          <DialogFooter className="shrink-0 flex-row items-center justify-between gap-2 border-t border-border px-3 py-2.5 sm:justify-between sm:space-x-0">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Button type="submit" className="h-8 min-w-32 gap-1.5 text-[11px]" disabled={submitting || !allowSubmit}>
