...
export default function MyTasksPage() {
  ...
  const unreadTaskNotifications =
    notificationsData?.unreadTaskNotifications ||
    (notificationsData?.notifications || []).filter(
      (notification) => !notification.isRead && notification.task,
    );
  ...
  useEffect(() => {
    ...
    const unreadForTask = unreadTaskNotifications.filter(
      (notification) =>
        (String(notification.task) === selectedKey ||
          String(notification.taskId) === selectedKey) &&
        !markedNotificationIdsRef.current.has(String(notification._id)),
    );
    if (!unreadForTask.length) return;

    unreadForTask.forEach((notification) =>
      markedNotificationIdsRef.current.add(String(notification._id)),
    );
    void Promise.allSettled(
      unreadForTask.map((notification) =>
        EmployeeV2Service.markNotificationRead(notification._id),
      ),
    ).then(() =>
  ...
  const tasksQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await EmployeeV2Service.getMyTasksV2({
        ...
        relatedType:
          filters.relatedType !== "all" ? filters.relatedType : undefined,
  ...
  const selectedTask = useMemo(
    () =>
      tasks.find(
        (task) => task._id === selectedTaskId || task.taskId === selectedTaskId,
      ),
  ...
  const detailQuery = useQuery({
    ...
    queryFn: async () => {
      const response =
        await EmployeeV2Service.getMyTaskV2Detail(selectedTaskId);
  ...
  const createTaskMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      toast.success(
        `${createdTasks.length} task${createdTasks.length === 1 ? "" : "s"} created successfully`,
      );
  ...
  const editTaskMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        error.response?.data?.message || "Unable to update task details",
      ),
  });
  ...
  const reviewTaskMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(error.response?.data?.message || "Unable to record review"),
  });
  ...
  const respondMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        error.response?.data?.message || "Unable to respond to request",
      ),
  });
  ...
  const dueDateChangeMutation = useMutation({
    ...
    onSuccess: (response) => {
      const requestStatus =
        response.data?.data?.task?.dueDateChangeRequest?.status;
      toast.success(
    ...
    onError: (error) =>
      toast.error(
        error.response?.data?.message || "Unable to request due date change",
      ),
  });
  ...
  const dueDateChangeRespondMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        error.response?.data?.message || "Unable to respond to due date change",
      ),
  });
  ...
  const withdrawMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(
        error.response?.data?.message || "Unable to withdraw request",
      ),
  });
  ...
  const discussionMutation = useMutation({
    mutationFn: (payload) =>
      EmployeeV2Service.addTaskDiscussionMessage(payload),
    onSuccess: () => {
      invalidateTaskQueries();
    },
    onError: (error) =>
  ...
  const escalateMutation = useMutation({
    ...
    onError: (error) =>
      toast.error(error.response?.data?.message || "Unable to escalate task"),
  });
  ...
  const metricCards = [
    ...
    [
      "Pending Acceptance",
      employeeTabCounts.pending_acceptance,
      "Tasks",
      ClipboardList,
      "violet",
    ],
    [
      "Awaiting Review",
      analytics.tabCounts?.awaiting_review || 0,
      "Tasks",
      Eye,
      "blue",
    ],
    [
      ...
      analytics.tabCounts?.completed || 0,
      "Tasks",
      CheckSquare,
      "green",
    ],
  ...
  const taskTabs = tabs.map(([value, label, icon]) => ({
    ...
    notificationCount:
      value === "completed"
        ? undefined
        : (employeeTabCounts[value] ??
          analytics.tabCounts?.[SCOPE_BY_TAB[value] || value]),
  }));

  const navigate = useNavigate();

  const openRelatedItem = (task) => {
    const relatedTo = task.relatedTo || {};
    const relatedId = relatedTo.refId?._id || relatedTo.refId;
    const isEvent = relatedTo.type === "Event" || relatedTo.refModel === "Event";

    if (!isEvent || !relatedId) {
      openTaskDetail(task);
      return;
    }

    const taskKeys = new Set([task._id, task.taskId].filter(Boolean).map(String));
    const unreadForTask = unreadTaskNotifications.filter(
      (notification) =>
        (taskKeys.has(String(notification.task)) ||
          taskKeys.has(String(notification.taskId))) &&
        !markedNotificationIdsRef.current.has(String(notification._id)),
    );
    unreadForTask.forEach((notification) =>
      markedNotificationIdsRef.current.add(String(notification._id)),
    );
    if (unreadForTask.length) {
      void Promise.allSettled(
        unreadForTask.map((notification) =>
          EmployeeV2Service.markNotificationRead(notification._id),
        ),
    ...
    navigate(`/dashboard/assigned-events/${relatedId}`);
  };
          ...
          <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
              ...
              <Select
                ...
                onOpenChange={(isOpen) =>
                  setOpenFilterDropdown(isOpen ? "taskType" : null)
                }
              >
              ...
              <Select
                ...
                onOpenChange={(isOpen) =>
                  setOpenFilterDropdown(isOpen ? "status" : null)
                }
              >
              ...
              <Select
                ...
                onOpenChange={(isOpen) =>
                  setOpenFilterDropdown(isOpen ? "priority" : null)
                }
              >
                <SelectTrigger className="h-8 w-[135px] rounded-md text-xs">
                  <SelectValue />
              ...
              <Select
                ...
                onOpenChange={(isOpen) =>
                  setOpenFilterDropdown(isOpen ? "relatedType" : null)
                }
              >
                <SelectTrigger className="h-8 w-[150px] rounded-md text-xs">
                  <SelectValue />
            ...
            {tasksQuery.isFetching && !tasks.length ? (
              ...
              <DataTable
                    ...
                    : [
                        filters.tab === "pending_acceptance"
                          ...
                          : filters.tab === "requests_sent"
                            ? "Assigned To"
                            : ["collaborating", "awaiting_review"].includes(
                                  filters.tab,
                                )
                              ? "Primary Owner"
                              : "Assigned By",
                ...
                rows={tasks.map((task) => {
                  ...
                  const isTaskCreator =
                    String(task.createdBy) === String(currentEmployee?._id);
                  ...
                  const assignedByCell = (
                    <div className="ml-3 flex w-fit items-center gap-2 text-left">
                      <Avatar className="h-7 w-7">
                        <AvatarImage
                          src={getAvatarUrl(
                            task.createdByProfileImage?.smallUrl,
                          )}
                  ...
                  const assignedToCell = (
                    <div className="ml-3 flex w-fit items-center gap-2 text-left">
                      <Avatar className="h-7 w-7">
                        <AvatarImage
                          src={getAvatarUrl(
                            task.assignedToProfileImage?.smallUrl,
                          )}
                  ...
                  return [
                    <button
                      type="button"
                      className="ml-3 flex w-fit items-center gap-3 text-left"
                      ...
                      <span>
                        ...
                        {filters.tab === "collaborating" ? (
                          <span className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
                            ...
                            {task.collaborators?.length || 0} collaborator
                    ...
                    <button
                      ...
                      title={
                        task.relatedTo?.type === "Event"
                          ? `Open ${task.relatedTo?.name || "event"}`
                          : undefined
                      }
                    >
            ...
            <div className="flex flex-col gap-2 border-t border-border p-2 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-xs text-muted-foreground">
                ...
                of {pagination?.total ?? tasks.length} tasks
              </p>
            ...
            <div className="flex items-start gap-2 border-t border-border bg-blue-50 p-2 text-xs text-blue-700 dark:bg-blue-400/10 dark:text-blue-300">
              ...
              <p>
                Company tasks, work requests, self tasks and system-linked work
                appear here. Keep your progress updated and submit work for
                review on time.
              </p>
            </div>
          </section>
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className="flex max-h-[94vh] w-[calc(100%-1.5rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-[880px]">
          <TaskDetailDialog
            ...
            deleteTaskMutation={
              ["Super Admin", "Admin"].includes(currentEmployee?.accessRole)
                ? deleteTaskMutation
                : null
            }
          />
